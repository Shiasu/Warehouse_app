...
{
    public class SoldViewModel : MainViewModel
    {
        public DataTable SoldGoodsDataDB = MainModel.Select("SELECT [dbo].[Goods].[id], [dbo].[Categories].[Category], [dbo].[Goods].[Name]" +
                                                         "FROM [dbo].[Goods]" +
                                                         "JOIN [dbo].[Categories]" +
                                                         "ON [dbo].[Goods].[Category_id] = [dbo].[Categories].[id]" +
                                                         "WHERE [dbo].[Goods].[Status_id] = 3");

        public void LoadOnWarehouseGoodsToList(DataTable data, ListView list)
        {
            for (int i = 0; i < data.Rows.Count; i++) // перебираем данные
            {
                MainModel.Goods dataSoldGoods = new MainModel.Goods() // создаём экземпляр класса
                {
                ...
                list.Items.Add(dataSoldGoods); // выводим строку в список
            }
        }
    }
}
